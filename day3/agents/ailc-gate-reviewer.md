...
description: Read-only pre-check for an AILC gate. Use when a bundle or brief is ready for a human gate (intake, contract, design, verify, promote, operate) and the lead wants a verdict table with evidence before signing. Do NOT use to write or fix code, and never to approve.
