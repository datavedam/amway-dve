...
> directory is `assignments/mcp/01-basic-tools` (this folder), not the
> project root:
> ```bash
> cd assignments/mcp/01-basic-tools
> ```
...
**Bash/zsh:**
```bash
amclaude mcp add assignment-01-basic-stdio -- uv run --directory $PWD python server.py
```
...
```bat
amclaude mcp add assignment-01-basic-stdio -- uv run --directory %CD% python server.py
```
...
```bash
amclaude mcp list
amclaude mcp get assignment-01-basic-stdio
```
...
```bash
amclaude mcp remove assignment-01-basic-stdio
```
...
remember to `cd assignments/mcp/01-basic-tools` in that terminal too:

**Bash/zsh:**
```bash
MCP_TRANSPORT=streamable-http MCP_PORT=8001 uv run --directory $PWD python server.py
```

**PowerShell:**
```powershell
$env:MCP_TRANSPORT="streamable-http"; $env:MCP_PORT="8001"; uv run --directory $PWD python server.py
```

**Command Prompt (cmd.exe):**
```bat
set MCP_TRANSPORT=streamable-http && set MCP_PORT=8001 && uv run --directory %CD% python server.py
```
...
```bash
amclaude mcp add --transport http assignment-01-basic-http http://127.0.0.1:8001/mcp
```
...
```bash
amclaude mcp list
amclaude mcp get assignment-01-basic-http
```
...
```bash
amclaude mcp remove assignment-01-basic-http
```
...
```bash
amclaude mcp list
amclaude mcp get assignment-01-basic-stdio    # or assignment-01-basic-http
```
